...
logger = create_logger("piper_overlap_actor", LOG_LEVEL)
...
def _create_actors(
    ...
    for pp_rank in range(num_actors):
        ...
        actor = PiperOverlapActor.options(num_gpus=1).remote(
            pp_rank,
            optim_class,
            world_size,
            num_mbs,
...
class PiperOverlapActor:
    def __init__(
        ...
        self.logger = create_logger("piper_overlap_actor", LOG_LEVEL)
    ...
    def _forward(self, stage_id: int, mb_idx: int, *deps):
        ...
        if self.profile:
            ...
            profiler.export_chrome_trace(f"log/{self.mode}/actor{self.global_rank}_fwd{stage_id}mb{mb_idx}_trace.json")
    ...
    def _backward(self, stage_id: int, mb_idx: int, *deps, loss_fn=None):
        ...
        if self.profile:
            ...
            profiler.__exit__(None, None, None)
            profiler.export_chrome_trace(f"log/{self.mode}/actor{self.global_rank}_bwd{stage_id}mb{mb_idx}_trace.json")
    ...
    def _forward_backward(self, fwd_stage_id: int, fwd_mb_idx: int, bwd_stage_id: int, bwd_mb_idx: int, *deps, loss_fn=None):
        ...
        if self.profile:
            ...
            profiler.export_chrome_trace(
                f"log/{self.mode}/actor{self.global_rank}_"
